...
func (s *Server) GetRatingHandler(w http.ResponseWriter, r *http.Request) {
	...
	u, err := user.GetRating(r.Context(), convert.Int32(page), convert.Int32(count))
	if err != nil {
		log.Errorf("GetRatingHandler/user.GetRating() err: %v", err)
		_, _ = w.Write(models.Response(1, err.Error(), nil).ToString())
		return
	}
	jsonData, err := user.SliceToJson(u...)
	if err != nil {
		log.Errorf("GetRatingHandler/user.SliceToJson() err: %v", err)
		_, _ = w.Write(models.Response(1, err.Error(), nil).ToString())
		return
	}
	_, _ = w.Write(models.Response(0, "", jsonData).ToString())
